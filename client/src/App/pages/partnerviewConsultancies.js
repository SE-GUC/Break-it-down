    ...
    constructor(props) {
      ...
      // Retrieves the list of items from the Express app
       getDescription = async () => {
        ...
        await fetch(`http://localhost:4000/api/partner/viewConsultancy/${PID}/${TID}`)
        .then(res => res.json())
                  ...
                  return (
                   <ListGroup>  
                    <div key={el.applicantID}>
                      <span>
                      <ListGroup.Item variant="info">consultancyID:  {el.consultancyID}</ListGroup.Item>
                      </span>
                      <span>
                      <ListGroup.Item variant="info">accepted:  { JSON.stringify(el.accepted) } {"                         "}{"          "}
                      <button type="button" class="btn btn-success">Accept Consultancy</button>
                      </ListGroup.Item>
                      </span>
                      <span>
                     <ListGroup.Item variant="info">assigned:  { JSON.stringify(el.assigned) }</ListGroup.Item>
                      </span>
                      <br />
                    </div>
                    </ListGroup>
