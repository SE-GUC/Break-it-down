...
// Get all Partners (Malak&Nour) MONGOUPDATED
router.get('/', (req, res) =>{	
	partner.find()
    .then(items=>res.json(items))});

//Get Specific Partner (Malak&Nour) MONGOUPDATED
router.get('/:id', (req, res) => {
    partner.findById(req.params.id)
          .then(p=> res.json(p))
          .catch(err=>res.status(404).json({ msg: 'No Partner with the id of ${req.params.id}'}))
  });

  // Create a new Partner (Malak&Nour) MONGOUPDATED
router.post('/', async(req, res) => {
    const {type, name, email ,phoneNumber ,address,field ,description ,partners, boardMembers,events }=req.body
    const p = await partner.findOne({email})
    if(p) return res.status(400).json({error: 'Email already exists'})
...
router.put('/:id', (req, res) => {
  ...
  */
 // Update partner (Malak&Nour) done except id non existent case
router.put('/:id',  (req,res) => {
	const updpartner = req.body;
  Partner.findById(req.params.id)
		.then(partner=> {
            partner.type = updpartner.type ? updpartner.type : partner.type;
            ...
            partner.field = updpartner.field ? updpartner.field : partner.field;
            partner.partners = updpartner.partners ? updpartner.partners : partner.partners;

            partner.description = updpartner.description ? updpartner.description : partner.description;
            partner.boardMembers = updpartner.boardMembers ? updpartner.boardMembers : partner.boardMembers; 
            partner.events = updpartner.events ? updpartner.events : partner.events;
            partner.save().then(res.json({ msg: 'partner successfully updated', partner })).catch(err=> res.status(404).json({ msg: 'No event with the id of ${req.params.id}'}));
          } )
	});
// Delete partner (Malak&Nour) MONGOUPDATED
