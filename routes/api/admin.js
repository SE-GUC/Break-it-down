...
//checkExpiryDate(todate)

//console.log("--------Partner--------");
//console.log(checkExpiryDatePartner(todate));
// console.log("--------Member--------")
// console.log(checkExpiryDateMember(todate))
// console.log("----------CS----------")
// console.log(checkExpiryDateCS(todate))
// console.log("----------CA----------")
// console.log(checkExpiryDateCA(todate))

// const job = cron.job('* * * * * *', () =>
//    console.log(checkExpiryDate(todate))
// );
// job.start()
  ...
  if (partner === null)
    ...
    if (t.wantsConsultant === false) {
      ...
      for (var i = 0; i < t.applicants.length; i++) {
        if (t.applicants[i].assigned === true) flag = false;
      }
      ...
      if (flag === true) {
        ...
        res.json("an applicant was already assigned to this task");
      }
    } else {
      res.json(
        "partner requested a consultancy agency to help with the task including assigning applicants to the task"
      );
    }
  }
});
...
router.put("/AssignConsultancyAgency/:idP/:idT", async (req, res) => {
  ...
  if (partner === null)
    ...
    const acceptedConsultancy = consultancies.find(
      consultancies => consultancies.accepted === true
    );
    const consultancyID = acceptedConsultancy.consultancyID;

    for (var i = 0; i < t.consultancies.length; i++) {
      if (t.consultancies[i].assigned === true) flag = false;
    }
    ...
    if (flag === true) {
      const f = await users.findOneAndUpdate(
        ...
        {
          $set: {
            "tasks.$[i].consultancies.$[j].assigned": true,
      ...
      res.json("a consultancy was already assigned to this task");
    }
  }
});
...
router.get("/getUnapprovedTasks", async (req, res) => {
  ...
  user.forEach(element => {
    ...
    if(Tasks === undefined || Tasks.length === 0){
      //res.json("no tasks were found")
      console.log("no tasks were found")
    }
    else {
      const unApprovedTasks = Tasks.filter(task => task.approved === false)
      //res.json(unApprovedTasks)
...
async function notify(Id, content) {
  const ID = ObjectId(Id);
  const user = await users.findOne(ID);
  if (user === null) {
    res.json("the database has no partner with the given ID");
