...
router.post("/addfacility/:idC", async (req, res) => {
  try {
    ...
    User.findOneAndUpdate(
      { _id: req.params.idC },
      { $addToSet: { facilities: newFacility } },
      (err, cospaceRooms) => {
        if (err) {
...
router.get("/viewCoworkingSpace/:idC", async (req, res) => {
  try {
    ...
    if (cospace === undefined)
      return res.json("Coworking space does not exist");
...
router.put("/updateRoom/:idc/:idr", async (req, res) => {
  try {
    ...
    if (!temp2) res.status(404).json({ msg: "Room does not exist." });
    ...
    if (req.body.capacity && req.body.roomNumber) {
      ...
      res.json({
        msg: "Room capacity updated successfully",
        data: updatedroom
      });
    } else if (req.body.capacity) {
      const updatedroom = await User.findOneAndUpdate(
...
router.delete("/deleteRoom/:idC/:idR", async (req, res) => {
  try {
    ...
    users.update(
      { _id: objectid(req.params.idC) },
      { $pull: { rooms: { _id: objectid(req.params.idR) } } },
      async function(err, model) {
        if (err) res.json(error);
        else res.status(200).json({ msg: "Deleted" });
      }
...
router.get("/viewRoomSchedule/:idC/:idR", async (req, res) => {
  try {
    ...
    if (requestedSchedule === undefined || requestedSchedule.length == 0)
      return res.send("No schedule is allocated for this room yet.");
...
router.delete("/deleteSchedule/:idC/:idR/:idS", async (req, res) => {
  try {
    ...
    users.findOneAndUpdate(
      { _id: idc, "rooms._id": idr, "rooms.schedule._id": ids },
      { $pull: { "rooms.$.schedule": { _id: ids } } },
      async function(err, model) {
...
router.put("/updateSchedule/:idc/:idr/:ids", async (req, res) => {
  try {
    ...
    var check;
    temp3[0].rooms.schedule.forEach(sch => {
      if (sch._id == req.params.ids) check = sch.reserved;
    });
    if (check === true)
      return res.json({
        msg:
          "This schedule is already reserved. Please come back and updated it when its reservation period ends."
      });
    else {
      if (req.body.Date && req.body.time && req.body.scheduleNumber) {
        ...
        const updatedroom = await User.findOneAndUpdate(
          ...
          {
            $set: {
              "rooms.$[i].schedule.$[j].scheduleNummber":
        ...
        res.json({
          msg: "Schedule number updated successfully",
          data: updatedroom
        });
      }
    }
        ...
        var mongoose = require("mongoose");
         ...
         const updtbooking = await User.findOneAndUpdate(
          ...
          {$set: {
              ...
              "RoomsBooked.$[i].roomName": myRoom.roomName
          }},
        {arrayFilters: [
